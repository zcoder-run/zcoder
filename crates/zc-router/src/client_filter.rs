...
/// Decides whether a base-originated message reaches the client that owns `client_wspace_id`.
///
...
///
/// A message whose `wspace_id` matches the client's is delivered. A message whose
/// `wspace_id` is the default is treated as unscoped and delivered; unscoped
...
mod tests {
	...
	fn make_msg(wspace_id: Id) -> RouterMsg {
		RouterMsg {
			msg_id: MsgId::new(1),
			wspace_id,
	...
	fn test_client_filter_matching_id_passes() -> Result<()> {
		// -- Setup & Fixtures
		let client_wspace_id = Id::try_from("00000000-0000-0000-0000-000000000001".to_string())?;
		let msg = make_msg(client_wspace_id);

		// -- Exec
		let pass = client_filter(client_wspace_id, &msg);
	...
	fn test_client_filter_other_id_is_dropped() -> Result<()> {
		// -- Setup & Fixtures
		let client_wspace_id = Id::try_from("00000000-0000-0000-0000-000000000001".to_string())?;
		let other_wspace_id = Id::try_from("00000000-0000-0000-0000-000000000002".to_string())?;
		let msg = make_msg(other_wspace_id);
