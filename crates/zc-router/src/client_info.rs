...
///
/// `wks_dir` is the workspace root and the sole identity key. `label` is display
/// metadata for server logs and listings; it is derived from `wks_dir` when not
...
impl ClientInfo {
	...
	pub fn from_wks_dir(wks_dir: impl Into<String>) -> Self {
		let wks_dir = wks_dir.into();
		let label = label_from_dir(&wks_dir);
		Self { wks_dir, label }
	}
...
fn label_from_dir(wks_dir: &str) -> Option<String> {
	let parts: Vec<String> = Path::new(wks_dir)
		.components()
...
mod tests {
	...
	fn test_client_info_serde_roundtrip() -> Result<()> {
		...
		// -- Check
		assert_eq!(back.wks_dir, "/home/dev/zcoder");
	...
	fn test_client_info_label_derivation() -> Result<()> {
		...
		// -- Single component
		let info = ClientInfo::from_wks_dir("zcoder");
		assert_eq!(info.label.as_deref(), Some("zcoder"));

		// -- Root path
		let info = ClientInfo::from_wks_dir("/");
	...
	fn test_client_info_with_label_override() -> Result<()> {
		// -- Exec
		let info = ClientInfo::from_wks_dir("/home/dev/zcoder").with_label("custom");
